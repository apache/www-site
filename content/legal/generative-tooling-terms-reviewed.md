...
| OpenAI ChatGPT Team / Business, OpenAI API, Codex CLI | Public terms | You own the output. Note that ChatGPT Voice Output is for non-commercial use only; this does not affect code. | 2026-09-26 |
...
| Cursor | Hobby, Pro, Ultra | Teams (if under MSA) are not covered. If you enable Grok Bot, its separate terms also apply and are not covered here. | 2026-09-26 |
...
| Claude via AWS Bedrock | Pay-per-use | Output is Your Content, and Anthropic claims no ownership of prompts or outputs. Bedrock models that attach Provenance Data are Category X, see below; nothing indicates Claude's text or code output carries any. | 2026-09-26 |
| Amazon Q Developer | Free | Output is Your Content. AWS may use Free-tier content, including generated code, to improve its services and train models unless you opt out (a data-use issue, not an output-rights one). | 2026-09-26 |
| Amazon Q Developer | Pro | Output is Your Content. Unlike the Free tier, Pro content is not used for service improvement or model training, and Pro is covered by AWS's generative AI indemnity. | 2026-09-26 |
| Claude via Gemini Enterprise Agent Platform (formerly Vertex AI) | Pay-per-use | Customer owns all Outputs; Anthropic assigns any right it has in them and may not train on Customer Content. The governing terms are dated March 2024, and the acceptance point is a click-through in the Model Garden console, so check the version shown to you at model-grant time. | 2026-08-14 |
| Google Gemini via Gemini Enterprise Agent Platform (formerly Vertex AI) | Pay-per-use, ordinary generated output | Generated Output is Customer Data; Google asserts no ownership in it. Grounding features are Category X, see below. | 2026-09-26 |
...
| Qwen3.8, self-hosted | Qwen3.8-27B (exact version only) | Unmodified Apache-2.0, copyright Alibaba Cloud, with nothing addressing generated output and no revenue or user threshold. The 2.4T flagship in the same family is licensed differently and is Category X, see below. | 2026-08-15 |
...
| Lovable | Free, paid | The terms say you own any AI output generated for you. Output from the optional AI Gateway, or from a model you connect, is governed by that provider's terms. Don't use the output to train a competing model, review it before relying on it in high-risk contexts, and don't pass it off as human-written where that would mislead; those are obligations on you, not on your code's recipients. Lovable may use your content for model training unless you opt out (a data-use issue, not an output-rights one). | 2026-09-26 |
...
| StackBlitz Bolt.new | All plans | StackBlitz assigns you any rights it has in AI Output, subject to third-party rights in the underlying models. StackBlitz may use Bolt content from individual accounts for model training and licensed datasets unless you opt out; Teams and Enterprise workspaces are excluded (a data-use issue, not an output-rights one). | 2026-09-26 |
...
| OpenCode | Zen / Go managed tiers | Output is assigned to you at the OpenCode layer; the connected model provider's terms still apply. The terms limit use of the service to your own internal use, not for the benefit of a third party; this review reads that as a bar on reselling or sharing access, not on contributing the output. | 2026-09-26 |
| Kilo Code | Kilo Pass, Teams | Suggestions are assigned to you; the connected AI model's terms still apply, and some model licences would be Category X. Teams under an Anaconda enterprise agreement or order form are not covered. Kilo takes a broad licence to uploaded customer data to improve its services (a data-use issue, not an output-rights one). | 2026-09-26 |
...
| AWS Bedrock | Models that attach Provenance Data, feature-specific only | Where AWS documentation says a model marks its output with metadata, digital signatures, or watermarks (for example Amazon Titan Image Generator), neither you nor any end user may alter or remove that Provenance Data. That obligation reaches downstream recipients. Ordinary text and code output is not covered by this finding. | 2026-09-26 |
| Google Gemini via Gemini Enterprise Agent Platform (formerly Vertex AI) | Grounding with Google Search, Web Grounding for Enterprise, Grounding with Google Maps, feature-specific only | Customer, end users, and third parties may not cache, syndicate, resell, analyse, or train on grounded results and search suggestions, and Google retains all rights in Maps data. The restriction is stated to bind end users and third parties, not only the customer. Ordinary text and code generation is not covered by this finding. | 2026-09-26 |
| Devin Desktop (formerly Windsurf) (Cognition) | Free, Pro, Teams | Internal-business-purpose grant, competing-products restriction, and export/sanctions conditions stated to apply to output. | 2026-08-11 |
...
| Qwen3.8 2.4T flagship, self-hosted | Qwen3.8-2.4T-A95B and the FP8 variant (exact versions only; published under the parameter-count name, not the Max product name) | A custom licence, not a standard open source one. Running a model-as-a-service or AI coding assistant business needs a separate licence from Alibaba once revenue passes US$50 million over twelve months, and any product above 100 million monthly active users or US$20 million monthly revenue must display the model name in its user interface. | 2026-08-15 |
